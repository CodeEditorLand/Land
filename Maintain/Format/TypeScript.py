...
# File stem suffixes that indicate generated or compiled artefacts.
# Files matching these patterns are skipped even when their parent
# directory is not excluded (e.g. Source/Effect/*/Implementation/*.ts).
# Mirrors the file-level exclusions in .prettierignore.
GeneratedSuffix = frozenset(
    {
        "Generated",
        "BridgeShapeGenerated",
    }
)

# Exact filenames that are compiled copies of an authored .ts sibling.
# Mirrors the corresponding entries in .prettierignore.
CompiledJsExclude = frozenset(
    {
        "ESBuild",
    }
)

# Configuration/ESBuild/ directories contain authored .ts files alongside
# compiled .js copies. Skip any .js file that has a same-named .ts sibling.
JsConfigDirNames = frozenset(
    {
        "ESBuild",
    }
)
...
def Main() -> None:
    ...
    if Args.All:
        Target = sorted(
            ...
            and File.stem not in CompiledJsExclude
            and not any(Suffix in File.stem for Suffix in GeneratedSuffix)
            and not (
                File.suffix == ".js"
                and File.parent.name in JsConfigDirNames
                and File.with_suffix(".ts").exists()
            )
        )
    else:
