...
export class MultiCursorSession {
    ...
    _getNextMatch() {
        ...
        const nextMatch = this._editor.getModel().findNextMatch(this.searchText, lastAddedSelection.getEndPosition(), false, this.matchCase, this.wholeWord ? this._editor.getOption(134) : null, false);
        if (!nextMatch) {
            return null;
        }
    ...
    _getPreviousMatch() {
        ...
        const allSelections = this._editor.getSelections();
        const lastAddedSelection = allSelections[allSelections.length - 1];
        const previousMatch = this._editor.getModel().findPreviousMatch(this.searchText, lastAddedSelection.getStartPosition(), false, this.matchCase, this.wholeWord ? this._editor.getOption(134) : null, false);
        if (!previousMatch) {
            return null;
        }
    ...
    selectAll(searchScope) {
        ...
        if (searchScope) {
            return editorModel.findMatches(this.searchText, searchScope, false, this.matchCase, this.wholeWord ? this._editor.getOption(134) : null, false, 1073741824);
        }
        return editorModel.findMatches(this.searchText, true, false, this.matchCase, this.wholeWord ? this._editor.getOption(134) : null, false, 1073741824);
    }
}
    ...
    selectAll(findController) {
        ...
        if (findState.isRevealed && findState.searchString.length > 0 && findState.isRegex) {
            ...
            if (findState.searchScope) {
                matches = editorModel.findMatches(findState.searchString, findState.searchScope, findState.isRegex, findState.matchCase, findState.wholeWord ? this._editor.getOption(134) : null, false, 1073741824);
            }
            else {
                matches = editorModel.findMatches(findState.searchString, true, findState.isRegex, findState.matchCase, findState.wholeWord ? this._editor.getOption(134) : null, false, 1073741824);
            }
        }
        else {
            this._beginSessionIfNeeded(findController);
...
let SelectionHighlighter = class SelectionHighlighter extends Disposable {
    ...
    constructor(editor, _languageFeaturesService) {
        ...
        this.editor = editor;
        this._isEnabled = editor.getOption(111);
    ...
    static _createState(oldState, isEnabled, editor) {
        ...
        return new SelectionHighlighterState(editor.getModel(), r.searchText, r.matchCase, r.wholeWord ? editor.getOption(134) : null, oldState);
    }
    _setState(newState) {
        this.state = newState;
        if (!this.state) {
        ...
        const occurrenceHighlighting = this.editor.getOption(83) !== 'off';
