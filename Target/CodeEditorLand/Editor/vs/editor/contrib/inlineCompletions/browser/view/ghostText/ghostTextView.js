...
let GhostTextView = class GhostTextView extends Disposable {
    constructor(_editor, _model, _languageService) {
        ...
        this._useSyntaxHighlighting = this._editorObs.getOption(64).map(v => v.syntaxHighlightingEnabled);
...
function renderLines(domNode, tabSize, lines, opts) {
    ...
    const stopRenderingLineAfter = opts.get(120);
    const renderWhitespace = 'none';
    const renderControlCharacters = opts.get(97);
    const fontLigatures = opts.get(53);
    const fontInfo = opts.get(52);
    const lineHeight = opts.get(69);
