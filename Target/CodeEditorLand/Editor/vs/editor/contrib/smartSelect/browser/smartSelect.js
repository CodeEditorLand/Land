...
let SmartSelectController = class SmartSelectController {
    ...
    async run(forward) {
        ...
        if (!this._state) {
            await provideSelectionRanges(this._languageFeaturesService.selectionRangeProvider, model, selections.map(s => s.getPosition()), this._editor.getOption(117 /* EditorOption.smartSelect */), CancellationToken.None).then(ranges => {
