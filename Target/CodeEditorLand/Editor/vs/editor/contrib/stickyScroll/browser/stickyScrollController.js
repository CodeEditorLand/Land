...
let StickyScrollController = class StickyScrollController extends Disposable {
    ...
    _toggleFoldingRegionForLine(line) {
        ...
        const scrollTop = (foldingIcon.isCollapsed ?
            ...
            - this._editor.getOption(69 /* EditorOption.lineHeight */) * stickyLine.index + 1;
        this._editor.setScrollTop(scrollTop);
        this._renderStickyScroll(line);
    }
    _readConfiguration() {
        const options = this._editor.getOption(119 /* EditorOption.stickyScroll */);
        ...
        const lineNumberOption = this._editor.getOption(70 /* EditorOption.lineNumbers */);
    ...
    _readConfigurationChange(event) {
        if (event.hasChanged(119 /* EditorOption.stickyScroll */)
            || event.hasChanged(75 /* EditorOption.minimap */)
            || event.hasChanged(69 /* EditorOption.lineHeight */)
            || event.hasChanged(114 /* EditorOption.showFoldingControls */)
            || event.hasChanged(70 /* EditorOption.lineNumbers */)) {
            this._readConfiguration();
        }
        if (event.hasChanged(70 /* EditorOption.lineNumbers */)) {
            this._renderStickyScroll(0);
        }
    }
    _needsUpdate(event) {
    ...
    _onDidResize() {
        ...
        const theoreticalLines = layoutInfo.height / this._editor.getOption(69 /* EditorOption.lineHeight */);
    ...
    findScrollWidgetState() {
        const lineHeight = this._editor.getOption(69 /* EditorOption.lineHeight */);
        const maxNumberStickyLines = Math.min(this._maxStickyLines, this._editor.getOption(119 /* EditorOption.stickyScroll */).maxLineCount);
