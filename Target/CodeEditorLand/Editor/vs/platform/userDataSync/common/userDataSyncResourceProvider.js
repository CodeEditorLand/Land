...
let UserDataSyncResourceProviderService = class UserDataSyncResourceProviderService {
    ...
    async getRemoteSyncedProfiles() {
        const userData = await this.userDataSyncStoreService.readResource("profiles", null, undefined);
        if (userData.content) {
            const syncData = this.parseSyncData(userData.content, "profiles");
            return parseUserDataProfilesManifest(syncData);
        }
        return [];
    }
    async getLocalSyncedProfiles(location) {
        const refs = await this.userDataSyncLocalStoreService.getAllResourceRefs("profiles", undefined, location);
        if (refs.length) {
            const content = await this.userDataSyncLocalStoreService.resolveResourceContent("profiles", refs[0].ref, undefined, location);
            if (content) {
                const syncData = this.parseSyncData(content, "profiles");
                return parseUserDataProfilesManifest(syncData);
            }
        }
        return [];
    ...
    async getAssociatedResources({ uri }) {
        ...
        switch (resolved.syncResource) {
            case "settings": return this.getSettingsAssociatedResources(uri, profile);
            case "keybindings": return this.getKeybindingsAssociatedResources(uri, profile);
            case "tasks": return this.getTasksAssociatedResources(uri, profile);
            case "snippets": return this.getSnippetsAssociatedResources(uri, profile);
            case "globalState": return this.getGlobalStateAssociatedResources(uri, profile);
            case "extensions": return this.getExtensionsAssociatedResources(uri, profile);
            case "profiles": return this.getProfilesAssociatedResources(uri, profile);
            case "workspaceState": return [];
        }
    }
    ...
    resolveNodeContent(syncResource, content, node) {
        ...
        switch (syncResource) {
            case "settings": return this.resolveSettingsNodeContent(syncData, node);
            case "keybindings": return this.resolveKeybindingsNodeContent(syncData, node);
            case "tasks": return this.resolveTasksNodeContent(syncData, node);
            case "snippets": return this.resolveSnippetsNodeContent(syncData, node);
            case "globalState": return this.resolveGlobalStateNodeContent(syncData, node);
            case "extensions": return this.resolveExtensionsNodeContent(syncData, node);
            case "profiles": return this.resolveProfileNodeContent(syncData, node);
            case "workspaceState": return null;
        }
    }
    async resolveLatestContent(syncResource, profileId) {
        ...
        switch (syncResource) {
            case "globalState": return this.resolveLatestGlobalStateContent(profile);
            case "extensions": return this.resolveLatestExtensionsContent(profile);
            case "profiles": return this.resolveLatestProfilesContent(profile);
            case "settings": return null;
            case "keybindings": return null;
            case "tasks": return null;
            case "snippets": return null;
            case "workspaceState": return null;
        }
    }
    ...
    async getSnippetsAssociatedResources(uri, profile) {
        ...
        if (content) {
            const syncData = this.parseSyncData(content, "snippets");
            if (syncData) {
