...
let TerminalProcess = class TerminalProcess extends Disposable {
    ...
    constructor(shellLaunchConfig, cwd, cols, rows, env, _executableEnv, _options, _logService, _productService) {
        ...
        if (isWindows) {
            ...
            this.onProcessReady(e => {
                ...
                this._register(this._windowsShellHelper.onShellTypeChanged(e => this._onDidChangeProperty.fire({ type: "shellType", value: e })));
                this._register(this._windowsShellHelper.onShellNameChanged(e => this._onDidChangeProperty.fire({ type: "title", value: e })));
            });
        }
    ...
    async setupPtyProcess(shellLaunchConfig, options, shellIntegrationInjection) {
        ...
        ptyProcess.onData(data => {
            ...
            if (!this._isPtyPaused && this._unacknowledgedCharCount > 100000) {
                this._logService.trace(`Flow control: Pause (${this._unacknowledgedCharCount} > ${100000})`);
    ...
    async _throttleKillSpawn() {
        ...
        while (Date.now() - TerminalProcess_1._lastKillOrStart < 250) {
            ...
            await timeout(250 - (Date.now() - TerminalProcess_1._lastKillOrStart) + 50);
        }
    ...
    _sendProcessTitle(ptyProcess) {
        ...
        if (sanitizedTitle.toLowerCase().startsWith('python')) {
            this._onDidChangeProperty.fire({ type: "shellType", value: "python" });
        }
        else if (sanitizedTitle.toLowerCase().startsWith('julia')) {
            this._onDidChangeProperty.fire({ type: "shellType", value: "julia" });
        }
        else {
    ...
    acknowledgeDataEvent(charCount) {
        ...
        if (this._isPtyPaused && this._unacknowledgedCharCount < 5000) {
            this._logService.trace(`Flow control: Resume (${this._unacknowledgedCharCount} < ${5000})`);
