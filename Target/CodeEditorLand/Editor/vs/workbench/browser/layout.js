...
export class Layout extends Disposable {
    ...
    initLayoutState(lifecycleService, fileService) {
        ...
        const isNewWindow = lifecycleService.startupKind === 1 /* StartupKind.NewWindow */;
        const activityBarNotDefault = this.configurationService.getValue("workbench.activityBar.location" /* LayoutSettings.ACTIVITY_BAR_LOCATION */) !== "default" /* ActivityBarPosition.DEFAULT */;
        ...
        // Side bar is hidden and a new window is opened with activity bar not visible (not default)
            ...
            const viewContainerToRestore = this.storageService.get(SidebarPart.activeViewletSettingsKey, 1 /* StorageScope.WORKSPACE */, this.viewDescriptorService.getDefaultViewContainer(0 /* ViewContainerLocation.Sidebar */)?.id);
            if (viewContainerToRestore) {
                this.state.initialization.views.containerToRestore.sideBar = viewContainerToRestore;
