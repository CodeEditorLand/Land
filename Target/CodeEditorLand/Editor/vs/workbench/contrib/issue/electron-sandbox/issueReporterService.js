...
import { IIssueMainService, IProcessMainService } from '../../../../platform/issue/common/issue.js';
...
let IssueReporter = class IssueReporter extends Disposable {
    constructor(configuration, nativeHostService, issueMainService, processMainService) {
        ...
        this.issueReporterModel = new IssueReporterModel({
            ...configuration.data,
            issueType: configuration.data.issueType || 0 /* OldIssueType.Bug */,
            versionInfo: {
        ...
        if (configuration.data.issueType === 1 /* OldIssueType.PerformanceIssue */) {
    ...
    setEventHandlers() {
        this.addEventListener('issue-type', 'change', (event) => {
            ...
            if (issueType === 1 /* OldIssueType.PerformanceIssue */ && !this.receivedPerformanceInfo) {
                this.processMainService.$getPerformanceInfo().then(info => {
                    this.updatePerformanceInfo(info);
                });
            }
    ...
    isPreviewEnabled() {
        ...
        if (issueType === 0 /* OldIssueType.Bug */ && this.receivedSystemInfo) {
            return true;
        }
        if (issueType === 1 /* OldIssueType.PerformanceIssue */ && this.receivedSystemInfo && this.receivedPerformanceInfo) {
            return true;
        }
        if (issueType === 2 /* OldIssueType.FeatureRequest */) {
            return true;
        }
        return false;
    }
    ...
    setUpTypes() {
        ...
        reset(typeSelect, makeOption(0 /* OldIssueType.Bug */, localize('bugReporter', "Bug Report")), makeOption(2 /* OldIssueType.FeatureRequest */, localize('featureRequest', "Feature Request")), makeOption(1 /* OldIssueType.PerformanceIssue */, localize('performanceIssue', "Performance Issue (freeze, slow, crash)")));
    ...
    renderBlocks() {
        ...
        if (issueType === 0 /* OldIssueType.Bug */) {
            if (!fileOnMarketplace) {
                show(blockContainer);
                show(systemBlock);
        ...
        else if (issueType === 1 /* OldIssueType.PerformanceIssue */) {
            if (!fileOnMarketplace) {
                show(blockContainer);
                show(systemBlock);
                show(processBlock);
        ...
        else if (issueType === 2 /* OldIssueType.FeatureRequest */) {
...
IssueReporter = __decorate([
    ...
    __param(2, IIssueMainService),
    __param(3, IProcessMainService),
