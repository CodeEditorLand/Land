...
const REMOTE_TUNNEL_EXTENSION_TIMEOUT = 4 * 60 * 1000;
...
let RemoteTunnelWorkbenchContribution = class RemoteTunnelWorkbenchContribution extends Disposable {
    ...
    async recommendRemoteExtensionIfNeeded() {
        ...
        const shouldRecommend = async () => {
            if (this.storageService.getBoolean(REMOTE_TUNNEL_EXTENSION_RECOMMENDED_KEY, -1)) {
                return false;
            }
            if (await this.extensionService.getExtension(remoteExtension.extensionId)) {
                return false;
            }
            const usedOnHostMessage = this.storageService.get(REMOTE_TUNNEL_USED_STORAGE_KEY, -1);
                ...
                actions: {
                    primary: [
                        ...
                        new Action('doNotShowAgain', localize('action.doNotShowAgain', "Do not show again"), undefined, true, () => {
                            this.storageService.store(REMOTE_TUNNEL_EXTENSION_RECOMMENDED_KEY, true, -1, 0);
                        }),
                    ]
                }
            });
    ...
    async startTunnel(asService) {
        ...
        this.storageService.store(REMOTE_TUNNEL_HAS_USED_BEFORE, true, -1, 1);
        ...
        this._register(registerAction2(class extends Action2 {
            ...
            async run(accessor) {
                ...
                if (!didNotifyPreview) {
                    ...
                    storageService.store(REMOTE_TUNNEL_PROMPTED_PREVIEW_STORAGE_KEY, true, -1, 0);
                }
                const disposables = new DisposableStore();
                ...
                const connectionInfo = await that.startTunnel(asService);
                if (connectionInfo) {
                    ...
                    storageService.store(REMOTE_TUNNEL_USED_STORAGE_KEY, JSON.stringify(usedOnHostMessage), -1, 0);
                }
                else {
                    notificationService.notify({
                        severity: Severity.Info,
...
workbenchRegistry.registerWorkbenchContribution(RemoteTunnelWorkbenchContribution, 3);
