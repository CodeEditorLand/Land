...
export const ContextKeys = {
    SCMViewMode: new RawContextKey('scmViewMode', "list"),
    SCMViewSortKey: new RawContextKey('scmViewSortKey', "path"),
...
class SetListViewModeAction extends ViewAction {
    constructor(id = 'workbench.scm.action.setListViewMode', menu = {}) {
        super({
            ...
            toggled: ContextKeys.SCMViewMode.isEqualTo("list"),
...
class SetListViewModeNavigationAction extends SetListViewModeAction {
    constructor() {
        super('workbench.scm.action.setListViewModeNavigation', {
            id: MenuId.SCMTitle,
            when: ContextKeyExpr.and(ContextKeyExpr.equals('view', VIEW_PANE_ID), ContextKeys.RepositoryCount.notEqualsTo(0), ContextKeys.SCMViewMode.isEqualTo("tree")),
            group: 'navigation',
            order: -1000
        });
    }
...
class SetTreeViewModeAction extends ViewAction {
    constructor(id = 'workbench.scm.action.setTreeViewMode', menu = {}) {
        super({
            ...
            toggled: ContextKeys.SCMViewMode.isEqualTo("tree"),
            menu: { id: Menus.ViewSort, group: '1_viewmode', ...menu }
        });
    }
    async runInView(_, view) {
...
class SetTreeViewModeNavigationAction extends SetTreeViewModeAction {
    constructor() {
        super('workbench.scm.action.setTreeViewModeNavigation', {
            id: MenuId.SCMTitle,
            when: ContextKeyExpr.and(ContextKeyExpr.equals('view', VIEW_PANE_ID), ContextKeys.RepositoryCount.notEqualsTo(0), ContextKeys.SCMViewMode.isEqualTo("list")),
            group: 'navigation',
            order: -1000
        });
    }
...
class SetSortKeyAction extends ViewAction {
    constructor(sortKey, title) {
        super({
            ...
            precondition: ContextKeys.SCMViewMode.isEqualTo("list"),
    ...
    async runAction(action) {
        try {
            ...
            if (this._runningActions.size === 0) {
                this.storageService.store("scm.input.lastActionId", action.id, 0, 0);
            }
        }
    }
};
...
let SCMInputWidget = class SCMInputWidget {
    ...
    set input(input) {
        ...
        if (this.configurationService.getValue('editor.wordBasedSuggestions', { resource: textModel.uri }) !== 'off') {
            this.configurationService.updateValue('editor.wordBasedSuggestions', 'off', { resource: textModel.uri }, 8);
        }
    ...
    getContentHeight() {
        const lineHeight = this.inputEditor.getOption(69);
        const { top, bottom } = this.inputEditor.getOption(86);
    ...
    renderValidation() {
        this.clearValidation();
        this.element.classList.toggle('validation-info', this.validation?.type === 2);
        this.element.classList.toggle('validation-warning', this.validation?.type === 1);
        this.element.classList.toggle('validation-error', this.validation?.type === 0);
        ...
        this.validationContextView = this.contextViewService.showContextView({
            ...
            render: container => {
                ...
                validationContainer.classList.toggle('validation-info', this.validation.type === 2);
                validationContainer.classList.toggle('validation-warning', this.validation.type === 1);
                validationContainer.classList.toggle('validation-error', this.validation.type === 0);
...
let SCMViewPane = class SCMViewPane extends ViewPane {
    ...
    set viewMode(mode) {
        ...
        this.storageService.store(`scm.viewMode`, mode, 1, 0);
    }
    ...
    set viewSortKey(sortKey) {
        ...
        if (this._viewMode === "list") {
            this.storageService.store(`scm.viewSortKey`, sortKey, 1, 0);
        }
    }
    constructor(options, commandService, editorService, menuService, scmService, scmViewService, storageService, uriIdentityService, keybindingService, themeService, contextMenuService, instantiationService, viewDescriptorService, configurationService, contextKeyService, openerService, telemetryService, hoverService) {
        ...
        this.storageService.onDidChangeValue(1, undefined, this.disposables)(e => {
            switch (e.key) {
                case 'scm.viewMode':
        ...
        this.revealResourceThrottler.queue(() => this.treeOperationSequencer.queue(async () => {
            for (const repository of this.scmViewService.visibleRepositories) {
                ...
                for (let j = repository.provider.groups.length - 1; j >= 0; j--) {
                    ...
                    const resource = this.viewMode === "tree"
    ...
    getViewMode() {
        ...
        const storageMode = this.storageService.get(`scm.viewMode`, 1);
    ...
    getViewSortKey() {
        ...
        const storageSortKey = this.storageService.get(`scm.viewSortKey`, 1);
    ...
    loadTreeViewState() {
        const storageViewState = this.storageService.get('scm.viewState2', 1);
        if (!storageViewState) {
            return undefined;
        }
        try {
    ...
    storeTreeViewState() {
        if (this.tree) {
            this.storageService.store('scm.viewState2', JSON.stringify(this.tree.getViewState()), 1, 1);
        }
    }
    ...
    updateIndentStyles(theme) {
        this.treeContainer.classList.toggle('list-view-mode', this.viewMode === "list");
        this.treeContainer.classList.toggle('tree-view-mode', this.viewMode === "tree");
