...
let SCMActiveRepositoryController = class SCMActiveRepositoryController extends Disposable {
    ...
    _getRepositoryResourceCount(repository) {
        return observableFromEvent(this, repository.provider.onDidChangeResources, () => getRepositoryResourceCount(repository.provider));
    }
    ...
    _updateStatusBar(repository, commands, store) {
        ...
        for (let index = 0; index < commands.length; index++) {
            ...
            store.add(index === 0 ?
                this.statusbarService.addEntry(statusbarEntry, `status.scm.${index}`, 0, 10000) :
                this.statusbarService.addEntry(statusbarEntry, `status.scm.${index}`, 0, { id: `status.scm.${index - 1}`, alignment: 1, compact: true }));
        }
    }
