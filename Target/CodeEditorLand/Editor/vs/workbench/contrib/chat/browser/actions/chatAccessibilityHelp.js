...
export class ChatAccessibilityHelp {
    constructor() {
        this.priority = 107;
        this.name = 'panelChat';
        this.type = "help";
        this.when = ContextKeyExpr.and(CONTEXT_CHAT_LOCATION.isEqualTo(ChatAgentLocation.Panel), ContextKeyExpr.or(CONTEXT_IN_CHAT_SESSION, CONTEXT_RESPONSE, CONTEXT_REQUEST));
    }
...
export function getAccessibilityHelpText(type) {
    const content = [];
    if (type === 'panelChat') {
...
export function getChatAccessibilityHelpProvider(accessor, editor, type) {
    ...
    return new AccessibleContentProvider(type === 'panelChat' ? "panelChat" : "inlineChat", { type: "help" }, () => helpText, () => {
