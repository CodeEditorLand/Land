...
This document outlines the implementation approach for adding comprehensive health monitoring and auto-restart capabilities to the Cocoon extension host process.
...
2. Process handle is dropped after initialization (line 327: `let _process = Some(ChildProcess);`)
...
**Location**: [`Element/Mountain/Source/IPC/Common/HealthStatus.rs`](../../Element/Mountain/Source/IPC/Common/HealthStatus.rs)

**Available Components**:
- ✅ `HealthMonitor` - Health state tracking with scoring (0-100)
- ✅ `SeverityLevel` - Priority levels (Low, Medium, High, Critical)
- ✅ `HealthIssue` - Types of detectable issues (HighLatency, MemoryPressure, ConnectionLoss, etc.)
...
1. **Activate Process State Management**
   - Store `CocoonProcessState` in a shared, thread-safe container (e.g., `Arc<Mutex<CocoonProcessState>>`)
   - Update state on process lifecycle events (spawn, health check, crash, restart)
...
3. **Implement State Transitions**
   - Track: `Idle` → `Spawning` → `Running` → `Unhealthy` → `Restarting` → `Terminated`
...
**Objective**: Calculate and maintain health scores using existing infrastructure
...
2. **Restart Limits**:
   ...
   - After 3 failed restarts: Mark as permanently failed, require manual intervention
...
### File: [`Element/Mountain/Source/ProcessManagement/CocoonManagement.rs`](../../Element/Mountain/Source/ProcessManagement/CocoonManagement.rs)
